...
std::vector<std::string> currentanswers(4);
...
void getAnswers(int questionNum){
    int correct = rand() % 4;
    int temp;
    ...
    for (int i=0; i < 4; i++) {
        ...
        if (i == correct)
            currentanswers[i] = answers[questionNum];
        else {
            while (temp == questionNum)
                temp = rand() % answers.size();

            currentanswers[i] = answers[temp];
        }
    }
}
...
void printQuestion(int questionNum){
    ...
    for (int i=0; i < 4; i++)
        std::cout << (char)(97+i) << ") " << currentanswers[i] << std::endl;
}
